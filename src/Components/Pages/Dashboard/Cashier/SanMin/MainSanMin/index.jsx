...
import AddSanMin from "../AddSanMin";
import UpdateSanMin from "../UpdateSanMin";
const { Search } = Input;
...
const MainSanMin = () => {
  ...
  const handleDeleteHepatit = (obj) => {
    confirm({
      ...
      .then(async () => {
        await SanMinProvider.deleteSanMin(obj.id);
        setContract((p) =>
          p.filter((reg) => {
  ...
  useEffect(() => {
    ...
    body.pageSize = 20;
    SanMinProvider.getAllSanMinBody(body)
      .then((res) => {
  ...
  const downloadExcel = () => {
    ...
    body.pageNum = currentPage;
    body.pageSize = 20;
    SanMinProvider.downloadExcel(body)
      .then((res) => {
        console.log(res);
  ...
  const onchangeStatus = () => {
    SanMinProvider.changePaymentStatus(contractId, paymentStatus)
      .then((res) => {
  ...
  return (
    <VirusMainWrapper>
      <div className="top">
        <h3 className="col-2">San.minimum</h3>
      ...
      >
        <AddSanMin onCloseModal={onCloseModal} />
      </Drawer>
      <Drawer
        anchor={"right"}
        open={isOpenModal2}
      ...
      >
        <UpdateSanMin onCloseModal2={onCloseModal2} editContract={editContract} />
      </Drawer>
