...
const UpdateParasite = ({ onCloseModal2, editParasite }) => {
  ...
  useEffect(() => {
    setValue("parasiteName", editParasite.parasite_name);
  }, [editParasite]);
  ...
  const onSubmitParasite = async (values) => {
    ...
    body.id = editParasite.id;

    setLoading(true);
    ParasiteProvider.updateParasite(body)
      .then((res) => {
        reset();
  ...
  return (
    <div>
      <ModalHeader className="modal-header">
        <h2 className="title">Parazit o`zgartirish</h2>
